...
class CuteGameHub:
    ...
    def main_menu(self):
        ...
        # Кнопка начать игру
        start_rect = self.draw_button("Начать игру", self.screen_width // 2, 250)

        # Кнопка музыки
        music_text = "Выкл. музыку" if self.music_enabled else "Вкл. музыку"
        music_rect = self.draw_button(music_text, self.screen_width // 2, 350)

        # Кнопка выхода
        exit_rect = self.draw_button("Выход", self.screen_width // 2, 450)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if music_rect.collidepoint(mouse_pos):
                    self.toggle_music()
                    return  # Выйти для обновления текста кнопки
    ...
    def run_memory_game(self):
        ...
        pairs_found = 0
        game_over = False
        waiting = False  # Флаг для задержки при несовпадении
        ...
        while running:
            ...
            for card in cards:
                if card['solved']:
                    color = (200, 255, 200)  # Светло-зеленый для найденных пар
                elif card['revealed']:
                    color = (255, 255, 200)  # Желтый для открытых карточек
                else:
                    color = self.BUTTON_COLOR  # Розовый для закрытых
                ...
                if card['revealed'] or card['solved']:
                    ...
                    pygame.draw.circle(self.screen, self.ACCENT_COLOR,
                                       card['rect'].center, 30, 3)

            # Кнопка назад
            back_rect = self.draw_button("Меню", self.screen_width // 2, 550)

            # Проверка завершения игры
            if pairs_found == len(symbols) and not game_over:
                game_over = True
                self.draw_text("Вы нашли все пары!", self.font_medium, self.ACCENT_COLOR,
                               self.screen_width // 2, 550)
                restart_rect = self.draw_button("Играть снова", self.screen_width // 2, 500)
                            ...
                            if not card['revealed'] and not card['solved'] and card['rect'].collidepoint(mouse_pos):
                                ...
                                if selected is None:
                                    ...
                                    if selected['value'] == card['value']:
                                        ...
                                        # Задержка перед закрытием карточек
                                        waiting = True
                                        pygame.time.set_timer(pygame.USEREVENT, 500)  # Таймер на 500 мс

                if event.type == pygame.USEREVENT and waiting:
                    # Закрываем несовпавшие карточки после задержки
                    for card in cards:
